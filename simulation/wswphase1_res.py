...
if __name__ == "__main__":
  ...
  fout = open('wswphase1.csv', 'wb')
  spamwriter = csv.writer(fout, dialect='excel')
  spamwriter.writerow(['tick', 'prey', 'predator', 'energy'])
  i = 1
  k = 0
  N = 30
  while i<700: # main loop
    simulation.update()
    n = len(simulation.env.prey)
    n2 = len(simulation.env.snake)
    ...
    spamwriter.writerow([str(i), str(n),str(n2), str(e)])
